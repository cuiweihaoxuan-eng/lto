...
| 3 | 申请录收弹窗（合同选择+收入计划选择+审批流程+申请事由+六到位入口） | P0 | DONE | | |
| 4 | 内层录收审批单列表展示 | P0 | DONE | | |
| 5 | 录收详情弹窗（合同/项目信息+已选收入计划+审批流程+审批单信息+六到位附件） | P0 | DONE | | |
| 6 | 审批流程选择（属地自交付/调用市公司能力）+流程图展示 | P1 | DONE | | |
| 7 | 节点2/3审批人员选择（需求部门会签+属地/地市部门会签） | P1 | DONE | | |
| 8 | 详情弹窗审批人员记录（按流程顺序展示+状态标签） | P1 | DONE | | |
...
| 字段名 | 类型 | 必填 | 默认值 | 来源 | 校验规则 | 展示形式 | 交互约束 |
|--------|------|------|--------|------|----------|----------|----------|
| 流程类型 | 枚举 | 是 | 根据创建人自动判断 | 用户选择/系统判断 | 非空 | 下拉选择（属地自交付/调用市公司能力） | 可编辑 |
| 流程图 | 图形 | 是 | - | 根据流程类型显示 | 无 | 并排展示两种流程，当前选中高亮 | 只读 |
| 节点2-需求部门会签人员 | 人员组 | 是 | - | 用户选择 | 至少选一人 | 人员标签卡片列表，可添加/删除 | 可编辑 |
| 节点3-属地/地市部门会签人员 | 人员组 | 是 | - | 用户选择 | 至少选一人 | 人员标签卡片列表，含节点类型标签（人力/财务/云中台/网运部） | 可编辑 |
...
| 字段名 | 类型 | 必填 | 默认值 | 来源 | 校验规则 | 展示形式 | 交互约束 |
|--------|------|------|--------|------|----------|----------|----------|
| 人员姓名 | 文本 | 是 | - | 接口返回 | 无 | 头像+姓名+部门+角色 | 只读 |
| 人员标签 | 标签 | 是 | - | 接口返回 | 无 | 彩色标签（需求部门/属地人力/属地财务/属地云中台/属地网运部/市人力/市财务/市云中台/市网运部） | 只读 |
| 选择状态 | 布尔 | 是 | 未选 | 用户点击 | 无 | 勾选框+蓝色边框高亮 | 可编辑 |

##### 审批人员标签颜色规范
| 节点类型 | 标签背景色 | 标签文字色 |
|----------|-----------|-----------|
| 需求部门 | bg-gray-100 | text-gray-700 |
| 人力 | bg-blue-100 | text-blue-700 |
| 财务 | bg-green-100 | text-green-700 |
| 云中台 | bg-orange-100 | text-orange-700 |
| 网运部 | bg-purple-100 | text-purple-700 |
...
| 字段名 | 类型 | 必填 | 默认值 | 来源 | 校验规则 | 展示形式 | 交互约束 |
|--------|------|------|--------|------|----------|----------|----------|
| 已选收入计划 | 表格 | 是 | - | 接口返回 | 无 | 勾选框+序号+产品收入项+业务类型+发票种类+税率+计划确认总金额(含税/不含税)+预计确认日期+收入触发系统+计划状态+摘要+类型 | 只读 |
| 合计金额 | 文本 | 是 | - | 计算得出 | 无 | 含税合计+不含税合计 | 只读 |

##### 审批流程区块（新增）
| 字段名 | 类型 | 必填 | 默认值 | 来源 | 校验规则 | 展示形式 | 交互约束 |
|--------|------|------|--------|------|----------|----------|----------|
| 审批流程 | 区块 | 是 | - | 接口返回 | 无 | 流程图+审批人员记录 | 只读 |
| 审批进度徽章 | 文本 | 是 | - | 计算得出 | 无 | "x/y 已审批"徽章，绿色=全部通过，黄色=进行中 | 只读 |
| 流程图 | 图形 | 是 | - | 接口返回 | 无 | 横向流程节点图，当前节点高亮蓝色 | 只读 |
| 审批人员记录 | 列表 | 是 | - | 接口返回 | 无 | 按流程顺序展示每个审批人，含状态图标+头像+姓名+部门+角色+节点标签+审批状态+审批时间+审批意见 | 只读 |

##### 审批人员记录卡片样式
| 审批状态 | 卡片背景 | 边框色 | 状态图标 |
|----------|---------|--------|----------|
| 已审批 | bg-green-50 | border-green-200 | 绿色勾 ✓ |
| 已驳回 | bg-red-50 | border-red-200 | 红色叉 ✗ |
| 待审批 | bg-yellow-50 | border-yellow-200 | 黄色时钟 ⏰ |
...
### v6 - 2026-05-18
- **审批流程新增**：
  - 申请录收弹窗新增审批流程选择（属地自交付/调用市公司能力），默认根据创建人自动判断
  - 流程图展示（两个流程并列，当前选中高亮）
- **审批人员选择新增**：
  - 节点2：需求部门会签人员（多选）
  - 节点3：属地/地市部门会签人员（多选，含人力/财务/云中台/网运部标签）
  - 选择人员弹窗（按节点过滤+人员标签展示）
- **录收详情弹窗新增**：
  - 已选收入计划区块（表格展示+合计金额）
  - 审批流程区块（流程图+审批人员记录）
  - 审批人员记录：按流程顺序展示已审批/待审批状态，含头像/姓名/标签/审批时间/意见
- **PRD更新**：
  - 删除六到位纵览区块（页面已移除）
  - 新增审批流程配置说明（流程图+人员标签颜色规范）
