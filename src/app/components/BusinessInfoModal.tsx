...
interface BusinessInfoModalProps {
  ...
  initialTab?: "detail" | "flow";
  onLinkOpportunity?: () => void;
}

export function BusinessInfoModal({ isOpen, onClose, data, initialTab = "detail", onLinkOpportunity }: BusinessInfoModalProps) {
  const [activeTab, setActiveTab] = useState<"detail" | "flow">(initialTab);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-[900px] max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        {/* 底部按钮 */}
        <div className="flex items-center justify-center gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50" onClick={e => e.stopPropagation()}>
          {activeTab === "flow" && (
            <Button>确定</Button>
          )}
          {data.businessInfoStatus === "未处理" && (
            <>
              <Button onClick={onLinkOpportunity}>关联商机</Button>
              <Button>创建商机</Button>
              <Button variant="outline">回退集团</Button>
              <Button variant="outline">取回</Button>
            </>
          )}
